...
<p align="center">
  ...
  薬のパッケージを読み取るか添付文書をアップロードし、音声またはテキストで資料に基づく回答を得られます。
</p>
...
- 中国語・英語の言語切替対応（AI応答とTTS音声の選択を同期）
...
設定済みのキーワードがユーザーの質問に含まれると、システムはリスクイベントを記録し、メールアラートを送信できます。
...
```
MedVision-RAG/
├── frontend/                  # Vue 3 CDNの単一ページWebアプリ
...
このリポジトリには現在ライセンスファイルが含まれていません。想定範囲外で配布または再利用する前に、明示的なライセンスを追加してください。
